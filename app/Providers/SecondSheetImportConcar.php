...
class SecondSheetImportConcar implements ToCollection, WithStartRow {
    ...
    public function collection(Collection $rows) {
        foreach ($rows as $row) {
            ...
            $secondBean = new SecondSheetImportConcar(0);
            $fecha_cargo = $row[0];
            $fecha_venc = $row[2];
            $fecha_pago = $row[4];
            $fecha_emi = $row[6];
...
//echo Date::excelToDateTimeObject($fecha_cargo)->format("Y-m-d");
            ...
            Contabilidad::carga_pagos_alexia_concar("tmp_pagos_alexia_concar_" . $this->id, [

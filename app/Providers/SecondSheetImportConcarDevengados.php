...
class SecondSheetImportConcarDevengados implements ToCollection, WithStartRow {
    ...
    public function collection(Collection $rows) {
        foreach ($rows as $row) {
            ...
            $secondBean = new SecondSheetImportConcarDevengados(0);
            $fecha_cargo = $row[0];
            $fecha_venc = $row[2];
            $fecha_pago = $row[4];
            $fecha_emi = $row[6];
...
//echo Date::excelToDateTimeObject($fecha_cargo)->format("Y-m-d");
            //if ($estado_compro != "DEVENGADO") {
            Contabilidad::carga_pagos_alexia_concar_devengados("tmp_pagos_alexia_concar_devengados_" . $this->id, [

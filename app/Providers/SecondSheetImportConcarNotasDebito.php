...
class SecondSheetImportConcarNotasDebito implements ToCollection, WithStartRow {
    ...
    public function collection(Collection $rows) {
        foreach ($rows as $row) {
            ...
            $secondBean = new SecondSheetImportConcarNotasDebito(0);
            $fecha_cargo = $row[0];
            $fecha_venc = $row[2];
            $fecha_pago = $row[4];
            $fecha_emi = $row[6];
...
//echo Date::excelToDateTimeObject($fecha_cargo)->format("Y-m-d");
            if (trim($tipo) === "ND BOL" || trim($tipo) === "ND FAC") {
                Contabilidad::carga_pagos_alexia_concar_notas_credito("tmp_pagos_alexia_concar_notasdebito_" . $this->id, [

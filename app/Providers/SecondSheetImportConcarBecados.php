...
class SecondSheetImportConcarBecados implements ToCollection, WithStartRow {
    ...
    public function collection(Collection $rows) {
        foreach ($rows as $row) {
            ...
            $secondBean = new SecondSheetImportConcarBecados(0);
            $fecha_cargo = $row[0];
            $fecha_venc = $row[2];
            $fecha_pago = $row[4];
            $fecha_emi = $row[6];
...
//echo Date::excelToDateTimeObject($fecha_cargo)->format("Y-m-d");
            if (trim($cancelado) === "0" && trim($tipo) === "BOL" && trim($estado_compro) === "COBRADO") {
                Contabilidad::carga_pagos_alexia_concar_becados("tmp_pagos_alexia_concar_becados_" . $this->id, [
